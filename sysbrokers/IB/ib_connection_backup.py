...
class connectionIB(object):
    ...
    def __init__(self, client_id, config_filename=None, log=None):
        """
        Safe initialisation of IB connection — allows use without config file.
        """
        from sysbrokers.IB import ib_connection_defaults

    self.client_id = client_id
    self.log = log or get_logger("connectionIB")

    # Wenn kein Config-Pfad übergeben wurde, Standardwerte setzen
    if config_filename is None:
        self.config = ib_connection_defaults.IB_DEFAULTS
        self.log.info("Using default IB connection settings (no config file).")
    else:
        try:
            self.config = parse_config(config_filename)
            self.log.info(f"Loaded IB connection config: {config_filename}")
        except Exception as e:
            self.log.warning(f"Could not load config file ({config_filename}): {e}")
            self.config = ib_connection_defaults.IB_DEFAULTS

    # IB-Standard-Initialisierung
    super().__init__()
